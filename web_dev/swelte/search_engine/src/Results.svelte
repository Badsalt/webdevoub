...
{#key $maxItemsPerPage}
    ...
    {#if json && "data" in json && "items" in json.data && json.data.items.length > 0 && !$IsHomePage}
        <div style="color: white; font-size: 1.5em;">
            Results Found: {json.data.items.length} Page: {currentPage}
        </div>
                ...
                {#if index < item_per_page * currentPage && index >= item_per_page * (currentPage - 1)}
                    ...
                    >
                        <div class="searchResults">
                            ...
                            {/if}
                            <h6 class="date">
                                Published: {processDate(item.published_at)}
                            </h6>
    ...
    {:else if json && !$IsHomePage}
        {#if json.hasOwnProperty("error")}
            <div class="searchResultsContainer">
                <p style="color: red; font-size: 2.5em" transition:fade>
                    {json.error}
                </p>
            </div>
        {:else if !$IsHomePage}
            <div class="searchResultsContainer">
                <p style="color: red; font-size: 3em" transition:fade>
                    No Results Found. You might like <a
                        style="color: blue;"
                        target="_blank"
                        href="https://youtu.be/dQw4w9WgXcQ">this</a
                    > instead
                </p>
            </div>
        {/if}
